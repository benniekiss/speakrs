...
impl LoadedSessions {
    pub(super) fn load(
        ...
        );
        {
            ...
            tracing::trace!(
                ...
                split_fbank_b32_ms = split_fbank_batched_elapsed.as_millis(),
                split_tail_ms = split_tail_elapsed.as_millis(),
                split_tail_b3_ms = split_tail_batched_elapsed.as_millis(),
                split_tail_b32_ms = split_primary_tail_batched_elapsed.as_millis(),
                ort_multi_mask_ms = multi_mask_elapsed.as_millis(),
                ort_multi_mask_b32_ms = multi_mask_batched_elapsed.as_millis(),
                total_ms,
    ...
    pub(super) fn into_model(self, model_path: &Path) -> Result<EmbeddingModel, ModelLoadError> {
